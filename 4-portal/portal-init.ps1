...
# === Step 3: 1-domains/ ディレクトリ + INDEX + CLAUDE.md 各層 ===
# 各 domain に CLAUDE.md を配置 → Claude Code が `cd 1-domains/<domain>` で
# その分野専用 context を auto-load (LLM 即参照容易化)

$domainContexts = @{
    "ai-development"   = "AI 駆動アプリ開発全般。LangChain / Claude API / 47 Skills / agent harness。主要 project: classweaver / mindgate / pet-care-app / ai-financial-office / agoora 本体。"
    "web-development"  = "Web フロント + バック。Next.js / Astro / React / Vue。Deploy: Vercel / さくら VPS / Cloudflare。主要: quard-web.jp / kuod-hp / pet-care-app frontend。"
    "data-engineering" = "データ基盤・ベクトル検索。PostgreSQL / SQLite WAL / ChromaDB (Phase D) / Tree-sitter (Phase 2)。~/.kb/ 46 repo + 1000+ Issue。"
    "devops"           = "CI/CD・監視・インフラ。GitHub Actions / Docker / Task Scheduler / BurntToast 通知。auto-relay workflow (Phase 2 KPI)。"
    "business"         = "QUARD 経営・営業・運営。Captain 業務領域 (sakura/dify/aomori-jichitai/audit)。月額管理 ¥152,734 改善余地。"
}
...
foreach ($domain in $domains.Keys) {
    ...
    $domainCtx = $domainContexts[$domain]
    if (-not $domainCtx) { $domainCtx = "(context TBD)" }
...
- ../CLAUDE.md で domain 全体 context
"@
        Write-File "$subPath\INDEX.md" $subIndex
    }
    Write-File "$domainPath\INDEX.md" $indexContent

    # === domain 専用 CLAUDE.md (LLM 即参照、Claude Code 起動時 auto-load) ===
    $domainClaude = @"
# CLAUDE.md — $domain ドメイン専用 context

@../../CLAUDE.md
@../../6-meta/PROFILE.md

`#$domain #domain-claude #context`

## このディレクトリの役割

$domainCtx

## 適用 R-rules (cluster 別)

- **D-A Multi-LLM Dispatch** (R55): 設計判断は必ず別 LLM で critic レビュー
- **D-D Autonomy & Planning** (R65): 24/48/72h plan + R10 batched + R20 auto-execute
- **D-G Pushback / Premise** (R19): Captain 前提を疑え、$domain で「絶対」「常に」と書く時は要 verbal check

## $domain で参照すべき主要 sub-domain

$(foreach ($sub in $domains[$domain]) { "- ``$sub/`` — ``$sub/INDEX.md`` 参照`n" })

## 質問が来たら

1. **R5 既存確認**: ``$domain`` 関連の過去 Issue を ~/.kb/issues/ で検索 (researcher 役)
2. **R8 反論余地**: 必ず 1 件以上 (critic 役)
3. **R10 一括承認**: 計画 + Stop conditions を Captain に提示
4. ハッシュタグ ``#$domain`` で本 portal 横断検索可

## 関連 agent (agents.yml)

- $domain 関連の researcher / architect / coder 役は 4-portal/agents.yml 参照
- ドメイン特化質問は domain-expert 役を呼出

最終更新: $dateStr
"@
    Write-File "$domainPath\CLAUDE.md" $domainClaude
}
